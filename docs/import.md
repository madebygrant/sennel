...
Takes the CSV that KeePassXC, Bitwarden and 1Password export. One reader rather than three, because
all three write a header row and different names for the same six things, so the columns are matched
by name. A tool nobody here has heard of works too if it names its columns like everyone else.
...
silence. A file with no title column is refused, and the message lists the header it found
instead.
...
and the line names it. Failing a whole import over one bad column would strand you mid-move.
